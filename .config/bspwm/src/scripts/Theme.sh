...
. "$HOME"/.config/bspwm/src/configs/theme.bash
...
apply_picom_config() {
	...
	picom_animations_file="$HOME/.config/picom/picom-animations.conf"
...
# Set dunst config
apply_dunst_config() {
	dunst_config_file="$HOME/.config/bspwm/src/configs/dunstrc"
...
apply_menu_colors() {
	# Rofi launchers
	cat >"$HOME"/.config/bspwm/src/rofi/shared.rasi <<-EOF
		* {
		    font: "${rofi_font}";
		    ...
		    img-background: url("${HOME}/.config/bspwm/src/assets/rofi.webp", width);
		}
	EOF

	# Screenlock colors
	sed -i "$HOME"/.config/bspwm/src/scripts/ScreenLocker \
		-e "s/bg=.*/bg=${sl_bg}/" \
		-e "s/fg=.*/fg=${sl_fg}/" \
...
apply_gtk_appearance() {
	...
	sed -i "$HOME"/.config/bspwm/src/configs/xsettingsd \
...
apply_wallpaper () {
	case $ENGINE in
		"Theme")
			feh -z --no-fehbg --bg-fill "${HOME}"/.config/bspwm/src/walls ;;
...
# Launch bars
apply_bar() {
	. "$HOME"/.config/bspwm/src/scripts/Bar.bash
}
